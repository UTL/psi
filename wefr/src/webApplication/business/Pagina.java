...
public class Pagina implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4591933129821816820L;
	...
	/**
	 * Metodo che permette di verificare se il nome del componente che si vuole
	 * inserire è univoco
	 * 
	 * @param c
	...
	public boolean controlloNome(Componente c) {
		boolean unico = true;
		for (int i = 0; i < moduli.size(); i++) { // scorre tutti i moduli
													// presenti nella pagina
			if ((moduli.get(i).getNome()).equals(c.getNome())) { // controlla il
																	// nome dei
																	// moduli
																	// esterni
				unico = false;
			...
			if ((moduli.get(i).getClass().getName())
				...
				for (int j = 0; j < cs.size(); j++) { // scorre tutti i
														...
														// moduli di tipo
														// ComponenteComposto
					if ((cs.get(j).getNome()).equals(c.getNome())) { // controlla
																		// l'univocità
																		// del
																		// nome
																		// nei
	...
	public boolean controlloNome(String nome) {
		...
		for (int i = 0; i < moduli.size(); i++) { // scorre tutti i moduli
													// presenti nella pagina
			if ((moduli.get(i).getNome()).equals(nome)) { // controlla il nome
															// dei moduli
															// esterni
				unico = false;
			}
			...
			if ((moduli.get(i).getClass().getName())
				...
				for (int j = 0; j < cs.size(); j++) { // scorre tutti i
														...
														// moduli di tipo
														// ComponenteComposto
					if ((cs.get(j).getNome()).equals(nome)) { // controlla
																// l'univocità
																// del nome nei
																// sottocomponenti
						unico = false;
	 ...
	 * 
	 * @param nome
	 * @return il componenteAlternative padre del componente il cui nome
	 *         corrisponde a quello cercato
	 */
		...
		for (int i = 0; i < moduli.size(); i++) { // scorre tutti i moduli
			...
			if ((moduli.get(i).getClass().getName())
				...
				for (int j = 0; j < cs.size(); j++) { // scorre tutte le
														...
														// moduli di tipo
														// ComponenteAlternativo
					if ((cs.get(j).getNome()).equals(nome)) { // cerca la
																// sottoAlternativa
																// con nome
																// uguale a
																// quello
																// cercato
						padre = (ComponenteAlternative) moduli.get(i); // ne
																		// recupera
																		// il
																		// padre
					}// if
	 ...
	 * 
	 * @param posizione
	 * @return il componente che all'interno della pagina occupa la posizione
	 *         cercata
	 */
	...
	public int cercaModulo(String n) {
		...
		for (int i = 0; i < moduli.size(); i++) { // scorre tutti i moduli
													// presenti nella pagina
			if ((moduli.get(i).getNome()).equals(n)) { // cerca il modulo con
														// nome uguale a quello
														// cercato
	...
	/**
	 * Metodo per la cancellazione dei vari componenti, sia semplici che
	 * composti che sottocomponenti
	 * 
	...
	public boolean cancellaModuloAll(String n, Boolean grafica) {
		...
		for (int i = 0; i < moduli.size(); i++) { // scorrimento di tutti i
													// moduli della pagina
			if ((moduli.get(i).getNome()).equals(n)) {
				app = i; // se l'elemento da cancellare è un modulo salvo la
			...
			if ((moduli.get(i).getClass().getName())
				...
				for (int j = 0; j < cs.size(); j++) { // scorrimento dei
					...
					if ((cs.get(j).getNome()).equals(n)) {
						cancellato = true;
						if (cs.size() == 1) { // controllo che il
												// comopnenteComposto non abbia
												// un solo sottoelemento
							if (!grafica) {
								if (JOptionPane
										.showConfirmDialog(
												null,
												"If you delete this component is also deleted the composite component. Continue?") == 0) { // nel
																																			// caso
																																			// non
																																			// lavoro
																																			// con
		...
		if (!cancellato && app != -1) { // nel caso in cui il componente che si
										// voleva eliminare è un modulo viene
										// effettuata la rimozione dell'intero
										// modulo
			moduli.remove(app);
			cancellato = true;
		}
	...
	/**
	 * Metodo per la ricerca di tutti i componenti anche figli di componenti
	 * composti o alternativi
	 * 
	...
	public Componente cercaModuloAll(String n) {
		...
		for (int i = 0; i < moduli.size(); i++) { // ricerca tra i moduli della
													// pagina
			if ((moduli.get(i).getNome()).equals(n)) {
				app = i; // salvo la posizione se viene trovato l'elemento
							// cercato
			}
			if ((moduli.get(i).getClass().getName())
					.equals("webApplication.business.ComponenteComposto")) {
